...
def bw_grid_vis_1(X, show=True, save=False, transform=False):
    ngrid = int(np.ceil(np.sqrt(len(X))))
    npxs_x = X.shape[2]
    npxs_y = X.shape[3]
    img = np.zeros((npxs_x * ngrid + ngrid - 1,
                    npxs_y * ngrid + ngrid - 1))
    for i, x in enumerate(X):
        j = i % ngrid
        i = i / ngrid
        if transform:
            x = transform(x)
        img[i*npxs_x+i:(i*npxs_x)+npxs_x+i, j*npxs_y+j:(j*npxs_y)+npxs_y+j] = x
    if show:
        plt.imshow(img, interpolation='nearest')
        plt.show()
    if save:
...
def bw_grid_vis_2(X, show=True, save=False, transform=False):
    ngrid = int(np.ceil(np.sqrt(len(X))))
    npxs_x = X.shape[2]+2
    npxs_y = X.shape[3]+2
    img = np.zeros((npxs_x * ngrid + ngrid - 1,
    ...
    for i, x in enumerate(X):
        ...
        if transform:
            x = transform(x)
        x_pad = np.zeros((x.shape[0]+2, x.shape[1]+2), dtype=np.float64)
        x_pad = np.lib.pad(x[0], ((1,1), (1,1)), 'constant', constant_values=int(x.max()))
        x = np.expand_dims(x_pad, axis=0)
        img[i*npxs_x+i:(i*npxs_x)+npxs_x+i, j*npxs_y+j:(j*npxs_y)+npxs_y+j] = x
    if show:
        plt.imshow(img, interpolation='nearest')
        plt.show()
...
def make_2d_paths(n_code, n_paths, n_steps=480):
    """
    create a random path through code space by interpolating between points
    """
    paths_1 = []
    paths_2 = []
    p_starts_1 = np.random.randn(n_paths, n_code)
    p_starts_2 = np.random.randn(n_paths, n_code)
    for i in range(n_steps/48):
        p_ends_1 = np.random.randn(n_paths, n_code)
        p_ends_2 = np.random.randn(n_paths, n_code)
        for weight in np.linspace(0., 1., 48):
            paths_1.append(p_starts_1*(1-weight) + p_ends_1*weight)
            paths_2.append(p_starts_2*(1-weight) + p_ends_2*weight)
        p_starts_1 = np.copy(p_ends_1)
        p_starts_2 = np.copy(p_ends_2)

    paths_1 = np.asarray(paths_1)
    paths_2 = np.asarray(paths_2)
    return paths_1, paths_2

def make_9d_paths(n_code, n_paths, n_steps=480):
    """
    create a random path through code space by interpolating between points
    """
    p_starts = np.random.randn(n_paths, n_code)
    ...
    for d in xrange(9):
        ...
        for i in range(n_steps/48):
            p_ends = np.random.randn(n_paths, n_code)
            for weight in np.linspace(0., 1., 48):
                paths_d.append(p_starts*(1-weight) + p_ends*weight)
            p_starts = np.copy(p_ends)
        paths_d = np.asarray(paths_d)
        paths_nd.append(paths_d)

    # paths_nd = np.asarray(paths_d)
    # paths_2 = np.asarray(paths_2)
    return paths_nd

def make_nd_paths(n_d, n_code, n_paths, n_steps=480):
    """
    create a random path through code space by interpolating between points
    """
    p_starts = np.random.randn(n_paths, n_code)
...
class ConvVAE(PickleMixin):
    ...
    # def _get_h2(self, X, w, w2, w3, b3, wmu, bmu, wsigma, bsigma):
    #     h = conv_and_pool(X, w)
    #     h2 = conv_and_pool(h, w2)
    #     h2_flatten = h2.reshape((h2.shape[0], -1))
    #     return h, h2, h2_flatten

    # def _get_deconv_dec(self, X, w, b, w2, b2, w3, wo):
    #     h = rectify(T.dot(X, w) + b)
    #     h2f = rectify(T.dot(h, w2) + b2)
    #
    #     h2 = h2f.reshape((h2f.shape[0], w3.shape[1], self.downpool_sz_h,
    #                     self.downpool_sz_w))
    #     h3 = deconv_and_depool(h2, w3)
    #     # h3 = depool(h2)
    #     y = deconv_and_depool(h3, wo, activation=hard_tanh)
    #     return h, h2f, h2, h3, y
    ...
    def _deconv_dec(self, X, w, b, w2, b2, w3, wo):
        ...
        # variable
        h2 = h2.reshape((h2.shape[0], w3.shape[1], self.downpool_sz_h,
                        self.downpool_sz_w))
        h3 = deconv_and_depool(h2, w3)
        y = deconv_and_depool(h3, wo, activation=hard_tanh)
        return y
        ...
        for e in range(epochs):
            ...
            if e == epochs or e % 10 == 0:
                ...
                if trX.shape[1] == 3:
                    ...
                    img1 = bw_grid_vis_1(x_rec, show=False)
                    img2 = bw_grid_vis_1(recs, show=False)
                    img3 = bw_grid_vis_1(samples, show=False)
                ...
                paths_1, paths_2 = make_2d_paths(self.n_code, 3)
                for i in range(paths_1.shape[1]):
                    #ipdb.set_trace()
                    path1_samples = self._x_given_z(floatX(paths_1[:, i, :]))
                    path2_samples = self._x_given_z(floatX(paths_2[:, i, :]))
                    for j, (sample1, sample2) in enumerate(zip(path1_samples, path2_samples)):
                        if trX.shape[1] == 3:
                            imsave(os.path.join(
                                samples_path, 'paths_%d_%d.png' % (i, j)),
                                np.concate((tf(sample1), tf(sample2)), axis=1))
                        else:
                            imsave(os.path.join(samples_path,
                                                'paths_%d_%d.png' % (i, j)),
                                np.concatenate((sample1.squeeze(), sample2.squeeze()), axis=1))
    def generate_2d(self, trX, generate_root):
        def tf(x):
            return ((x + 1.) / 2.).transpose(1, 2, 0)
        paths_1, paths_2 = make_2d_paths(self.n_code, 10)
        ...
        for i in range(paths_1.shape[1]):
            ...
            for j, (sample1, sample2) in enumerate(zip(path1_samples, path2_samples)):
                if trX.shape[1] == 3:
                    imsave(os.path.join(
                        generate_root, 'paths_%d_%d.png' % (i, j)),
                        np.concatenate((tf(sample1), tf(sample2)), axis=1))
                else:
                    imsave(os.path.join(generate_root,
                                        'paths_%d_%d.png' % (i, j)),
                           np.concatenate((sample1.squeeze(), sample2.squeeze()), axis=1))

    def generate_9d(self, trX, generate_root):
        def tf(x):
            return ((x + 1.) / 2.).transpose(1, 2, 0)
        paths_9d = make_9d_paths(self.n_code, 3)
        print 'generating data'
        for i in range(paths_9d[0].shape[1]):
            #ipdb.set_trace()
            print 'path %d' % i
            path_9d_samples = []
            for d in xrange(9):
                path_d_samples = self._x_given_z(floatX(paths_9d[d][:, i, :]))
                path_9d_samples.append(path_d_samples)
            # path1_samples = self._x_given_z(floatX(paths_9d[0][:, i, :]))
            # path2_samples = self._x_given_z(floatX(paths_9d[1][:, i, :]))
            for j in range(path_9d_samples[0].shape[0]):
                nd_samples = np.empty((trX.shape[2], 9*trX.shape[3]), dtype=np.float32)
                for d in xrange(9):
                    nd_samples[:, d*trX.shape[3]:(d+1)*trX.shape[3]] = path_9d_samples[d][j].squeeze()
                if trX.shape[1] == 3:
                    imsave(os.path.join(
                        generate_root, 'paths_%d_%d.png' % (i, j)),
                        nd_samples)
    ...
    def generate_nd(self, trX, generate_root, n_d):
        def tf(x):
            return ((x + 1.) / 2.).transpose(1, 2, 0)
        paths_nd = make_nd_paths(n_digit, self.n_code, 10)
        print 'generating data'
        for i in range(paths_nd[0].shape[1]):
            #ipdb.set_trace()
            print 'path %d' % i
            path_nd_samples = []
            for d in xrange(n_d):
                path_d_samples = self._x_given_z(floatX(paths_nd[d][:, i, :]))
                path_nd_samples.append(path_d_samples)
            # path1_samples = self._x_given_z(floatX(paths_9d[0][:, i, :]))
            # path2_samples = self._x_given_z(floatX(paths_9d[1][:, i, :]))
            generated_samples = np.empty((path_nd_samples[0].shape[0], 1, trX.shape[2], n_d*trX.shape[3]))
            for j in range(path_nd_samples[0].shape[0]):
                nd_samples = np.empty((trX.shape[2], n_d*trX.shape[3]), dtype=np.float32)
                for d in xrange(n_d):
                    nd_samples[:, d*trX.shape[3]:(d+1)*trX.shape[3]] = path_nd_samples[d][j].squeeze()
                generated_samples[j, 0, :, :] = nd_samples
                if trX.shape[1] == 3:
                    imsave(os.path.join(
                        generate_root, 'paths_%d_%d.png' % (i, j)),
                        nd_samples)
                    ...
                    imsave(os.path.join(generate_root,
                                        'paths_%d_%d.png' % (i, j)),
                           nd_samples)
            generated_img = bw_grid_vis_2(generated_samples, show=False)
            imsave(os.path.join(generate_root, 'generated_%d.png'%i), generated_img)
